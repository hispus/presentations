...
url=$(grep -Pzo '(?s)"dhis".*?\{.*?"baseurl":.*?".*?"' /opt/dhis2/dish.json | awk -F'"' '/baseurl/ {print $4}')
username=$(grep -Pzo '(?s)"dhis".*?\{.*?"username":.*?".*?"' /opt/dhis2/dish.json | awk -F'"' '/username/ {print $4}')
password=$(grep -Pzo '(?s)"dhis".*?\{.*?"password":.*?".*?"' /opt/dhis2/dish.json | awk -F'"' '/password/ {print $4}')
